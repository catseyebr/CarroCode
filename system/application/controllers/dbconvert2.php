...
class Dbconvert extends MyController {
	...
	function basic()
	{
		...
		$this->insert('Pessoafisica_model', array(
				...
				'cpf_nome' 			=> 'Eduardo',
				'cpf_sobrenome'		=> 'Assis',
				'cpf_numero' 		=> '92226515020',
				'cpf_sexo' 			=> 'm',
				'cpf_rg' 			=> '2064416874',
				'cpf_dtnasc'		=> '1975-10-05',
				'cpf_ativo' 		=> 't',
	...
	function carrega_locadoras(){
		...
		foreach ($this->old_locadoras as $oldloc_row){
			$inserttemploc = array(
				...
				'temploc_tlocst_id' 	=> 1,
				'temploc_nome' 			=> utf8_encode($oldloc_row['locadora']),
				'temploc_cnpj' 			=> utf8_encode($oldloc_row['cnpj']),
				'temploc_telefone' 		=> utf8_encode($oldloc_row['telefone']),
				'temploc_fax' 			=> utf8_encode($oldloc_row['fax']),
				'temploc_tollfree' 		=> utf8_encode($oldloc_row['tollfree']),
				'temploc_email' 		=> utf8_encode($oldloc_row['email']),
				'temploc_nomecon' 		=> utf8_encode($oldloc_row['nome']),
				'temploc_funcaocon' 	=> utf8_encode($oldloc_row['cargo']),
				'temploc_cep' 			=> (integer)str_replace(str_replace($oldloc_row['cep'],'.',''),'-',''),
				'temploc_rua' 			=> utf8_encode($oldloc_row['endereco']),
				'temploc_bairro' 		=> utf8_encode($oldloc_row['bairro']),
				'temploc_uf' 			=> (integer)$this->get('Estado_model', array('uf_abr' => $oldloc_row['estado']), 'GetEstadoId'),
				'temploc_cidade' 		=> (integer)$this->get('Cidade_model', array('city_nome' => utf8_encode($oldloc_row['cidade'])), 'GetCidadeId'),
			...
			if($oldloc_row['taxas']!=NULL){
				...
				$end_id = $this->insert('Endereco_model', array(
												...
												'end_city_id' 		=> ((integer)$this->get('Cidade_model', array('city_nome' => utf8_encode($oldloc_row['cidade'])), 'GetCidadeId')!=0) ? (integer)$this->get('Cidade_model', array('city_nome' => utf8_encode($oldloc_row['cidade'])), 'GetCidadeId'): NULL,
												'end_endereco' 		=> utf8_encode($oldloc_row['endereco']),
												'end_complemento'	=> '',
												'end_numero' 		=> utf8_encode($oldloc_row['endereco']),
												'end_bairro' 		=> utf8_encode($oldloc_row['bairro']),
												'end_latitude' 		=> '',
												'end_longitude'		=> '',)
											);
				}
				...
				if($oldloc_row['cnpj']!=NULL){										
					$cnpj_id = $this->insert('Pessoajuridica_model', array(
												...
												'cnpj_nomefantasia' => utf8_encode($oldloc_row['locadora']),
												'cnpj_razaosoc' 	=> NULL,
												'cnpj_numero' 		=> utf8_encode($oldloc_row['cnpj']),
												'cnpj_ie' 			=> NULL
												)
											);
				}
				...
				$loc_met_id = $this->insert('Metatags_model', array(
												'met_title' 		=> ($oldloc_row['meta_title']!='') ? utf8_encode($oldloc_row['meta_title']) : utf8_encode($oldloc_row['locadora']),
												'met_description' 	=> utf8_encode($oldloc_row['meta_description']),
												'met_keywords' 		=> utf8_encode($oldloc_row['meta_keywords']))
												);
				$dep_pj = $this->insert('Departamentopj_model', array(
												'deppj_dep_id' 		=> 1,
												'deppj_cnpj_id' 	=> (integer)$cnpj_id)
												);
				$nome = preg_split('/ /',utf8_encode($oldloc_row['nome']), 2);												
				$cpf_new = $this->insert('Pessoafisica_model', array(
												...
												'cpf_ativo' 		=> 't',
												'cpf_dthcadastro'	=> date("Y-m-d H:i:s"),
												'cpf_cargo'			=> utf8_encode($oldloc_row['cargo']))
											);
				$this->insert('Usuario_model', array(
				...
				if($oldloc_row['cartao_visa']!=0 || $oldloc_row['cartao_visa']!=NULL){
					$this->insert('Pjformapagamento_model', array(
												...
												'pjfrmpag_minima' 	=> utf8_encode($oldloc_row['parcela_min']),
												'pjfrmpag_parcelas' => utf8_encode($oldloc_row['num_parcelas']),
												'pjfrmpag_obs' 		=> utf8_encode($oldloc_row['cartoes']))
												);
				}
				...
				$insertloc = array(
								...
								'loc_nomelocadora'	=> utf8_encode($oldloc_row['locadora']),
								...
								'loc_txtprazocancel'=> utf8_encode($oldloc_row['prazo_cancel']),
								'loc_maisinfo'		=> utf8_encode($oldloc_row['mais_info']),
								'loc_online'		=> ($oldloc_row['xml_parser']==1)? 't' : 'f',
								'loc_kmrodado'		=> $oldloc_row['km_rodado'],
								'loc_obs'			=> utf8_encode($oldloc_row['obs_locadoras']),
								'loc_texto_livre'	=> utf8_encode($oldloc_row['texto_livre_locadoras']),
				...
				$this->insert('Pagina_model', array(
												...
												'pag_link_titulo' 	=> utf8_encode($oldloc_row['link_titulo']),
												'pag_class_tipo' 	=> 'f')
											);
				$this->insert('Pagina_model', array(
												'pag_proj_id' 		=> 2,
	...
	function carrega_lojas(){
		...
		foreach ($this->old_lojas as $oldloj_row){
			...
			$end_id = $this->insert('Endereco_model', array(
												...
												'end_city_id' 		=> ((integer)$this->get('Cidade_model', array('city_nome' => utf8_encode($oldloj_row['cidade']), 'city_uf_id' => $estado_id), 'GetCidadeId')!=0) ? (integer)$this->get('Cidade_model', array('city_nome' => utf8_encode($oldloj_row['cidade'])), 'GetCidadeId'): NULL,
												'end_endereco' 		=> utf8_encode($oldloj_row['endereco']),
												'end_complemento'	=> '',
												'end_numero' 		=> utf8_encode($oldloj_row['endereco']),
												'end_bairro' 		=> utf8_encode($oldloj_row['bairro']),
												'end_latitude' 		=> $oldloj_row['lat_geocode'],
												'end_longitude'		=> $oldloj_row['long_geocode'])
											);
			}
			$loj_met_id = $this->insert('Metatags_model', array(
												'met_title' 		=> (utf8_encode($oldloj_row['meta_title'])!='') ? utf8_encode($oldloj_row['meta_title']) : utf8_encode($oldloj_row['nome']),
												'met_description' 	=> utf8_encode($oldloj_row['meta_description']),
												'met_keywords' 		=> utf8_encode($oldloj_row['meta_keywords']))
												);
			$dep_pj = $this->insert('Departamentopj_model', array(
												'deppj_dep_id' 	=> 2,
												'deppj_cnpj_id' => (integer)$this->get('Locadora_model', array('loc_id' => $oldloj_row['id_locadoras']), 'GetLocCnpj'),
												'deppj_nome' 	=> utf8_encode($oldloj_row['nome']))
												);
			...
			$insertloj = array(
								...
								'loj_iata'		=> utf8_encode($oldloj_row['iata_loja']),
								'loj_aero'		=> ($oldloj_row['sigla']==1) ? 't' : 'f',
								'loj_permalink'	=> (utf8_encode($oldloj_row['link_name'])!='') ? utf8_encode($oldloj_row['link_name']) : utf8_encode(strtolower(str_replace(' ', '-', $oldloj_row['nome']))),
								'loj_classe'	=> (utf8_encode($oldloj_row['link_name'])!='') ? utf8_encode($oldloj_row['link_name']) : utf8_encode(strtolower(str_replace(' ', '-', $oldloj_row['nome']))),
								'loj_obs'		=> utf8_encode($oldloj_row['obs_lojas']),
								'loj_ativo'		=> ($oldloj_row['ativo']==1) ? 't' : 'f',
								'loj_valorextra'=> $oldloj_row['valor_extra'],
								'loj_texto_livre'=> utf8_encode($oldloj_row['texto_livre_lojas'])
								);
		...
		if($oldloj_row['dom']==1){
				...
				$this->insert('Pagina_model', array(
												...
												'pag_url' 		=> 'loja/'.utf8_encode($oldloj_row['link_name']),
	...
	function carrega_cidades(){
		...
		do{
			$city_met_id = $this->insert('Metatags_model', array(
												'met_title' 		=> utf8_encode($row_cidades['NOME_LOCAL']),
												'met_description' 	=> "Aluguel de carro em ".utf8_encode($row_cidades['NOME_LOCAL']).", ".$row_cidades['UF_LOG']." - "."Brasil",
												'met_keywords' 		=> utf8_encode($row_cidades['NOME_LOCAL']).", "
			...
			$this->insert('Cidade_model', array(
												...
												'city_pais_id' 	=> 1,
												'city_nome' 	=> utf8_encode($row_cidades['NOME_LOCAL']),
												'city_met_id' 	=> $city_met_id,
												'city_uf_id' 	=> (integer)$this->get('Estado_model', array('uf_abr' => $row_cidades['UF_LOG']), 'GetEstadoId'))
											);
			$tag_id = $this->insert('Tags_model', array('tag_nome' => utf8_encode($row_cidades['NOME_LOCAL'])));
	...
	function carrega_bairros(){
		...
		do{
			$this->insert('Bairro_model', array(
												...
												'bai_nome' 		=> utf8_encode($row_bairros['EXTENSO_BAI']),
												'bai_abr' 		=> utf8_encode($row_bairros['ABREV_BAI']))
											);
			$tagb_id = $this->insert('Tags_model', array('tag_nome' => utf8_encode($row_bairros['EXTENSO_BAI'])));
			$this->insert('Tagcidade_model', array(
	...
	function carrega_ceps(){
		...
		do{
			$this->insert('Cep_model', array(
												...
												'cep_rua' 		=> utf8_encode($row_ceps['NOME_LOG']),
												'cep_bairro' 	=> utf8_encode($row_ceps['CHVBAI1_LOG']),
												'cep_codigo' 	=> $row_ceps['CEP8_LOG'],
												'cep_compl' 	=> utf8_encode($row_ceps['COMPLE_LOG']),
												'cep_tipo' 		=> utf8_encode($row_ceps['ABREV_TIPO']),
	...
	function carrega_categorias(){
		...
		foreach ($this->old_cat as $oldcat_row){
			$cat_met_id = $this->insert('Metatags_model', array(
												'met_title' 		=> (utf8_encode($oldcat_row['meta_title'])!='') ? utf8_encode($oldcat_row['meta_title']) : utf8_encode($oldcat_row['descricao']),
												'met_description' 	=> utf8_encode($oldcat_row['meta_description']),
												'met_keywords' 		=> utf8_encode($oldcat_row['meta_keywords']))
												);
			$this->insert('Categoria_model', array(
												'cat_id' 		=> $oldcat_row['id_categoria'],
												'cat_met_id'	=> (integer)$cat_met_id,
												'cat_nome' 		=> utf8_encode($oldcat_row['descricao']),
												'cat_ordem' 	=> $oldcat_row['ordem_categoria'],
												'cat_desc' 		=> utf8_encode($oldcat_row['lt_desc_categoria']),
												'cat_permalink'	=> utf8_encode($oldcat_row['nome_categoria']),
												'cat_classe' 	=> utf8_encode($oldcat_row['nome_categoria']))
											);
			$this->insert('Pagina_model', array(
												'pag_proj_id' 	=> 1,
												'pag_met_id'	=> (integer)$cat_met_id,
												'pag_url' 		=> 'categoria/'.utf8_encode($oldcat_row['nome_categoria']),
												'pag_class' 	=> '/categoria/'.$oldcat_row['id_categoria'],
												'pag_link_titulo' 	=> utf8_encode($oldcat_row['descricao']),
												'pag_class_tipo' => 'f')
											);
			$this->insert('Pagina_model', array(
												'pag_proj_id' 	=> 2,
	...
	function carrega_carros(){
		...
		foreach ($this->old_car as $oldcar_row){
			$car_met_id = $this->insert('Metatags_model', array(
												'met_title' 		=> (utf8_encode($oldcar_row['meta_title'])!='') ? utf8_encode($oldcar_row['meta_title']) : utf8_encode($oldcar_row['modelo']),
												'met_description' 	=> utf8_encode($oldcar_row['meta_description']),
												'met_keywords' 		=> utf8_encode($oldcar_row['meta_keywords']))
												);
			$this->insert('Carro_model', array(
												'car_id' 			=> $oldcar_row['id_carros'],
												'car_met_id'		=> (integer)$car_met_id,
												'car_modelo' 		=> utf8_encode($oldcar_row['modelo']),
												...
												'car_cambio' 		=> utf8_encode($oldcar_row['cambio']),
												'car_desc' 			=> utf8_encode($oldcar_row['descCarro']),
												'car_linkyoutube' 	=> $oldcar_row['youtube_carros'],
												'car_txtyoutube' 	=> utf8_encode($oldcar_row['txt_auth_video']))
											);
			$this->insert('Pagina_model', array(
												'pag_proj_id' 	=> 1,
												'pag_met_id'	=> (integer)$car_met_id,
												'pag_url' 		=> 'carro/'.str_replace(' ', '-', strtolower(parent::RemoveAcentos(utf8_encode($oldcar_row['modelo'])))),
	...
	function carrega_grupos(){
		...
		foreach ($this->old_grp as $oldgrp_row){
			$this->insert('Grupo_model', array(
												...
												'grp_nome' 			=> utf8_encode($oldgrp_row['grupo']),
	...
	function carrega_protecoes(){
		...
		foreach ($this->old_prot as $oldprot_row){
			$this->insert('Protecao_model', array(
												...
												'prot_nome'					=> utf8_encode($oldprot_row['protecao']),
												'prot_desc' 				=> utf8_encode($oldprot_row['descricao']),
												'prot_franquia' 			=> utf8_encode($oldprot_row['franquia_protecao']),
												'prot_franquia_terceiros'	=> utf8_encode($oldprot_row['franquia_terceiros_protecao']),
