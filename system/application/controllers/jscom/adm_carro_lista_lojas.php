<?php
class Adm_Carro_Lista_Lojas extends MyController {
	
	public $get_vars;
	
 	function Adm_Carro_Lista_Lojas () {
  	...
  	public function index () {
		...
		$lojas_count = $this->get('loja_model', NULL);
		$count = count($lojas_count);
		if($count > 0){
		...
		if($search == TRUE){
			$lojas = $this->get('loja_model', array(
					'sortBy' => $sidx,
					'sortDirection' => $sord,
					'limit' => $limit,
					'offset' => $start,
		...
		foreach($lojas as $loj){
			echo "<row id='". $loj->loj_id."'>"; 
			echo "<cell>". $loj->loj_id."</cell>"; 
			echo "<cell><![CDATA[". $loj->loj_nome."]]></cell>"; 
			echo "</row>"; 
		} 
		echo "</rows>";
	}
