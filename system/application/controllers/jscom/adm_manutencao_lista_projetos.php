<?php
class Adm_Manutencao_Lista_Projetos extends MyController {
	
	public $get_vars;
	
 	function Adm_Manutencao_Lista_Projetos () {
  	...
  	public function index () {
		...
		$projetos_count = $this->get('projeto_model', NULL);
		$count = count($projetos_count);
		if($count > 0){
		...
		if($search == TRUE){
			$projetos = $this->get('projeto_model', array(
					'sortBy' => $sidx,
					'sortDirection' => $sord,
					'limit' => $limit,
					'offset' => $start,
		...
		foreach($projetos as $proj){
			echo "<row id='". $proj->proj_id."'>"; 
			echo "<cell>". $proj->proj_id."</cell>"; 
			echo "<cell><![CDATA[". $proj->proj_nome."]]></cell>"; 
			echo "</row>"; 
		} 
		echo "</rows>";
	}
