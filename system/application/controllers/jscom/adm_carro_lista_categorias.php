<?php
class Adm_Carro_Lista_Categorias extends MyController {
	
	public $get_vars;
	
 	function Adm_Carro_Lista_Categorias () {
  	...
  	public function index () {
		...
		$categorias_count = $this->get('categoria_model', NULL);
		$count = count($categorias_count);
		if($count > 0){
		...
		if($search == TRUE){
			$categorias = $this->get('categoria_model', array(
					'sortBy' => $sidx,
					'sortDirection' => $sord,
					'limit' => $limit,
					'offset' => $start,
		...
		foreach($categorias as $cat){
			echo "<row id='". $cat->cat_id."'>"; 
			echo "<cell>". $cat->cat_id."</cell>"; 
			echo "<cell><![CDATA[". $cat->cat_nome."]]></cell>"; 
			echo "</row>"; 
		} 
		echo "</rows>";
	}
