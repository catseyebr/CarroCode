<?php
class Adm_Manutencao_Lista_Paginas extends MyController {
	
	public $get_vars;
	
 	function Adm_Manutencao_Lista_Paginas () {
  	...
  	public function index () {
		...
		$paginas_count = $this->get('pagina_model', NULL);
		$count = count($paginas_count);
		if($count > 0){
		...
		if($search == TRUE){
			$paginas = $this->get('pagina_model', array(
					'sortBy' => $sidx,
					'sortDirection' => $sord,
					'limit' => $limit,
					'offset' => $start,
		...
		foreach($paginas as $proj){
			echo "<row id='". $proj->pag_id."'>"; 
			echo "<cell>". $proj->pag_id."</cell>"; 
			echo "<cell>". $proj->pag_proj_id."</cell>";
			echo "<cell><![CDATA[". $proj->pag_url."]]></cell>";
			echo "<cell><![CDATA[". $proj->pag_class."]]></cell>";
			echo "</row>"; 
		} 
		echo "</rows>";
	}
