<?php
class Adm_Carro_Lista_Grupos extends MyController {
	
	public $get_vars;
	
 	function Adm_Carro_Lista_Grupos () {
  	...
  	public function index () {
		...
		$grupos_count = $this->get('grupo_model', NULL);
		$count = count($grupos_count);
		if($count > 0){
		...
		if($search == TRUE){
			$grupos = $this->get('grupo_model', array(
					'sortBy' => $sidx,
					'sortDirection' => $sord,
					'limit' => $limit,
					'offset' => $start,
		...
		foreach($grupos as $grp){
			echo "<row id='". $grp->grp_id."'>"; 
			echo "<cell>". $grp->grp_id."</cell>"; 
			echo "<cell><![CDATA[". $grp->grp_nome."]]></cell>"; 
			echo "</row>"; 
		} 
		echo "</rows>";
	}
