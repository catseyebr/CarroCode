<?php
class Adm_Carro_Lista_Protecoes extends MyController {
	
	public $get_vars;
	
 	function Adm_Carro_Lista_Protecoes () {
  	...
  	public function index () {
		...
		$protecoes_count = $this->get('protecao_model', NULL);
		$count = count($protecoes_count);
		if($count > 0){
		...
		if($search == TRUE){
			$protecoes = $this->get('protecao_model', array(
					'sortBy' => $sidx,
					'sortDirection' => $sord,
					'limit' => $limit,
					'offset' => $start,
		...
		foreach($protecoes as $prot){
			echo "<row id='". $prot->prot_id."'>"; 
			echo "<cell>". $prot->prot_id."</cell>"; 
			echo "<cell><![CDATA[". $prot->prot_nome."]]></cell>"; 
			echo "</row>"; 
		} 
		echo "</rows>";
	}
