<?php
class Adm_Manutencao_Lista_Niveis extends MyController {
	
	public $get_vars;
	
 	function Adm_Manutencao_Lista_Niveis () {
  	...
  	public function index () {
		...
		$niveis_count = $this->get('niveis_model', NULL);
		$count = count($niveis_count);
		if($count > 0){
		...
		if($search == TRUE){
			$niveis = $this->get('niveis_model', array(
					'sortBy' => $sidx,
					'sortDirection' => $sord,
					'limit' => $limit,
					'offset' => $start,
		...
		foreach($niveis as $niv){
			echo "<row id='". $niv->niv_id."'>"; 
			echo "<cell>". $niv->niv_id."</cell>"; 
			echo "<cell><![CDATA[". $niv->niv_nome."]]></cell>"; 
			echo "</row>"; 
		} 
		echo "</rows>";
	}
