...
class Dbconvert extends MyController {
    ...
    function index()
	{
		...
		$this->data .= '<a href="/dbconvert/uf">Inserir Estados</a>';
		if($this->get('estado_model', array(NULL))){$this->data .= '<em>&nbsp;ok</em><br>';}else{$this->data .= '<br>';};
		...
		$this->data .= '<a href="/dbconvert/carrega_avaliacoes">Inserir Avaliações/Depoimentos</a>';
		if($this->get('avaliacao_model', array(NULL))){$this->data .= '<em>&nbsp;ok</em><br>';}else{$this->data .= '<br>';};
		$this->data .= '<hr>';
	...
	function basic()
	{
		...
		$this->insert('Dominio_model', array(
				'dom_proj_id' 	=> 1,
				'dom_name' 		=> 'www.alugueldecarrosonline.com'
			)
		);
		
		$this->insert('Dominio_model', array(
				'dom_proj_id' 	=> 1,
				'dom_name' 		=> 'alugueldecarrosonline.com'
			)
		);
		
		$this->insert('Dominio_model', array(
				'dom_proj_id' 	=> 2,
				'dom_name' 		=> 'www.carrentalsinbrazil.com'
			)
		);
		
		$this->insert('Dominio_model', array(
				'dom_proj_id' 	=> 2,
				'dom_name' 		=> 'carrentalsinbrazil.com'
			)
		);
		...
		$this->insert('Pagina_model', array(
				'pag_proj_id' 		=> 1,
				'pag_met_id'		=> $met_id,
				'pag_url' 			=> 'pesquisa/seleciona-loja',
				'pag_class' 		=> '/carro_pesquisa/selecionaLoja',
				'pag_class_tipo' 	=> 't'
			)
		);
		...
		$this->insert('Pagina_model', array(
				'pag_proj_id' 		=> 1,
				'pag_met_id'		=> $met_id,
				'pag_url' 			=> 'pesquisa/seleciona-seguro',
				'pag_class' 		=> '/carro_pesquisa/selecionaSeguro',
				'pag_class_tipo' 	=> 't'
			)
		);
		...
		$this->insert('Contatoref_model', array('conref_nome' => 'Celular'));
	...
	function carrega_locadoras(){
		...
		foreach ($this->old_locadoras as $oldloc_row){
			$estado_id = (integer)$this->get('Estado_model', array('uf_abr' => $oldloc_row['estado']), 'GetEstadoId');
			$inserttemploc = array(
			...
			if($oldloc_row['taxas']!=NULL){
				...
				$end_id = $this->insert('Endereco_model', array(
												...
												'end_city_id' 		=> ((integer)$this->get('Cidade_model', array('city_nome' => $oldloc_row['cidade'], 'city_uf_id' => $estado_id), 'GetCidadeId')!=0) ? (integer)$this->get('Cidade_model', array('city_nome' => $oldloc_row['cidade'], 'city_uf_id' => $estado_id), 'GetCidadeId'): NULL,
				...
				$this->insert('Pagina_model', array(
												...
												'pag_class' 		=> '/carro_locadora/carregaLocadora/'.$oldloc_row['id_locadora'],
				...
				$this->insert('Pagina_model', array(
												...
												'pag_met_id'		=> (integer)$loc_met_id,
												'pag_url' 			=> 'locadora/'.$oldloc_row['link_name'],
												'pag_class' 		=> '/layum_locadora/carregaLocadora/'.$oldloc_row['id_locadora'],
												'pag_link_titulo' 	=> $oldloc_row['link_titulo'],
												'pag_class_tipo' 	=> 'f')
											);
	...
	function carrega_lojas(){
		...
		foreach ($this->old_lojas as $oldloj_row){
			...
			$end_id = $this->insert('Endereco_model', array(
												...
												'end_city_id' 		=> ((integer)$this->get('Cidade_model', array('city_nome' => $oldloj_row['cidade'], 'city_uf_id' => $estado_id), 'GetCidadeId')!=0) ? (integer)$this->get('Cidade_model', array('city_nome' => $oldloj_row['cidade'], 'city_uf_id' => $estado_id), 'GetCidadeId'): NULL,
			...
			$insertloj = array(
								...
								'loj_city_id'	=> ((integer)$this->get('Cidade_model', array('city_nome' => $oldloj_row['cidade'], 'city_uf_id' => $estado_id), 'GetCidadeId')!=0) ? (integer)$this->get('Cidade_model', array('city_nome' => $oldloj_row['cidade'], 'city_uf_id' => $estado_id), 'GetCidadeId'): NULL,
								'loj_nome'		=> $oldloj_row['nome'],
								'loj_iata'		=> $oldloj_row['iata_loja'],
								'loj_aero'		=> ($oldloj_row['sigla']==1) ? 't' : 'f',
								'loj_permalink'	=> ($oldloj_row['link_name']!='') ? $oldloj_row['link_name'] : strtolower(str_replace(' ', '-', parent::RemoveAcentos($oldloj_row['nome']))),
								'loj_classe'	=> ($oldloj_row['link_name']!='') ? $oldloj_row['link_name'] : strtolower(str_replace(' ', '-', parent::RemoveAcentos($oldloj_row['nome']))),
		...
		if($oldloj_row['dom']==1){
				...
				$this->insert('Pagina_model', array(
												...
												'pag_class' 	=> '/carro_locadora/carregaLoja/'.$oldloj_row['id_loja'],
												'pag_class_tipo' => 'f')
											);
				$this->insert('Pagina_model', array(
												'pag_proj_id' 	=> 2,
												'pag_met_id'	=> (integer)$loj_met_id,
												'pag_url' 		=> 'loja/'.$oldloj_row['link_name'],
												'pag_class' 	=> '/layum_locadora/carregaLoja/'.$oldloj_row['id_loja'],
												'pag_class_tipo' => 'f')
											);
	...
	function carrega_grupos(){
		...
		foreach ($this->old_grp as $oldgrp_row){
			$this->insert('Grupo_model', array(
												...
												'grp_franquia' 				=> $oldgrp_row['franquia_grupos'],
												'grp_franquia_terceiros' 	=> $oldgrp_row['franquia_terceiros_grupos'],
												'grp_caucao' 				=> $oldgrp_row['valor_caucao'])
											);
				$this->insert('Projetoclasse_model', array(
						'projcla_proj_id' 	=> 1,
	...
	function carrega_avaliacoes(){
		mysql_select_db('layum', $this->old_db);
		$query_old_avaliacao = "SELECT * FROM loc_avaliacao";
		$lista_old_avaliacao = mysql_query($query_old_avaliacao, $this->old_db) or die(mysql_error());
		$row_oldavaliacao = mysql_fetch_assoc($lista_old_avaliacao);
		
		do{
			$this->old_avaliacao[] = $row_oldavaliacao;
		} while ($row_oldavaliacao = mysql_fetch_assoc($lista_old_avaliacao));
		
		foreach ($this->old_avaliacao as $oldavaliacao_row){
			$this->insert('Avaliacao_model', array(
					'aval_proj_id' 				=> 1,
					'aval_reserva_id'			=> $oldavaliacao_row['id_reserva'],
					'aval_cli_id'				=> $oldavaliacao_row['id_cliente'],
					'aval_dthcadastro'			=> $oldavaliacao_row['data'],
					'aval_lida'					=> ($oldavaliacao_row['status']==1)?'t':'f',
					'aval_w_acesso'				=> ($oldavaliacao_row['w_acesso']!='')?(integer)$this->get('Avaliacaoop_model', array('avalop_nome' => $oldavaliacao_row['w_acesso']), 'GetAvaliacaoOpId'):3,
					'aval_w_qualidade'			=> (integer)$this->get('Avaliacaoop_model', array('avalop_nome' => $oldavaliacao_row['w_qualidade']), 'GetAvaliacaoOpId'),
					'aval_w_conteudo'			=> (integer)$this->get('Avaliacaoop_model', array('avalop_nome' => $oldavaliacao_row['w_conteudo']), 'GetAvaliacaoOpId'),
					'aval_w_reserva'			=> $oldavaliacao_row['w_reserva'], 
					'aval_w_nota'				=> $oldavaliacao_row['w_nota'],
					'aval_sac_tempo'			=> (integer)$this->get('Avaliacaoop_model', array('avalop_nome' => $oldavaliacao_row['sac_tempo']), 'GetAvaliacaoOpId'),
					'aval_sac_info'				=> (integer)$this->get('Avaliacaoop_model', array('avalop_nome' => $oldavaliacao_row['sac_info']), 'GetAvaliacaoOpId'),
					'aval_sac_nota'				=> $oldavaliacao_row['sac_nota'],
					'aval_serv_retorno'			=> ($oldavaliacao_row['serv_retorno']=='Sim')?'t':'f',
					'aval_serv_indica'			=> ($oldavaliacao_row['serv_indica']=='Sim')?'t':'f',
					'aval_emp_local'			=> (integer)$this->get('Avaliacaoop_model', array('avalop_nome' => $oldavaliacao_row['emp_local']), 'GetAvaliacaoOpId'),
					'aval_emp_atendimento'		=> (integer)$this->get('Avaliacaoop_model', array('avalop_nome' => $oldavaliacao_row['emp_atendimento']), 'GetAvaliacaoOpId'),
					'aval_emp_horarios'			=> (integer)$this->get('Avaliacaoop_model', array('avalop_nome' => $oldavaliacao_row['emp_horarios']), 'GetAvaliacaoOpId'),
					'aval_emp_nota'				=> $oldavaliacao_row['emp_nota'],
					'aval_car_potencia'			=> (integer)$this->get('Avaliacaoop_model', array('avalop_nome' => $oldavaliacao_row['car_potencia']), 'GetAvaliacaoOpId'),
					'aval_car_consumo'			=> (integer)$this->get('Avaliacaoop_model', array('avalop_nome' => $oldavaliacao_row['car_consumo']), 'GetAvaliacaoOpId'),
					'aval_car_manutencao'		=> (integer)$this->get('Avaliacaoop_model', array('avalop_nome' => $oldavaliacao_row['car_manutencao']), 'GetAvaliacaoOpId'),
					'aval_car_seguranca'		=> (integer)$this->get('Avaliacaoop_model', array('avalop_nome' => $oldavaliacao_row['car_seguranca']), 'GetAvaliacaoOpId'),
					'aval_car_capacidade'		=> (integer)$this->get('Avaliacaoop_model', array('avalop_nome' => $oldavaliacao_row['car_capacidade']), 'GetAvaliacaoOpId'),
					'aval_car_limpeza'			=> (integer)$this->get('Avaliacaoop_model', array('avalop_nome' => $oldavaliacao_row['car_limpeza']), 'GetAvaliacaoOpId'),
					'aval_car_nota'				=> $oldavaliacao_row['car_nota'],
					'aval_depoimento'			=> $oldavaliacao_row['depoimento'],
					'aval_depoimento_resposta'	=> $oldavaliacao_row['depoimento_resposta'],
					'aval_mostra_site'			=> ($oldavaliacao_row['mostra_site']==1)?'t':'f',
					'aval_nome_depoimento'		=> $oldavaliacao_row['nome_depoimento']
				)
			);		
		}
		redirect('/dbconvert/index');
